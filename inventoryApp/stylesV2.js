...
//Evento para mostrar menu en Edit Product
document.addEventListener('click', function(event) {
    if (event.target.id.startsWith('editProductButton-')) {
      let buttonId = event.target.id; 
      showEditProductMenu(buttonId);
    }
  });
...
//Funcion para mostrar menu en Edit Product
function showEditProductMenu(buttonId) {
    // Extraer solo el número del id
    let productNumber = buttonId.replace('editProductButton-', '');
    // Buscar el menú de edición asociado a ese producto
    let editMenu = document.getElementById(`editMenu-${productNumber}`);
    let cancelButton = document.getElementById(`cancelEditProduct-${productNumber}`);
    let acceptButton = document.getElementById(`acceptEditProduct-${productNumber}`);
    // Si existe el menú, lo mostramos
    if (editMenu) {
      editMenu.style.display = 'flex'; 
    }
    // Mostramos los botones Cancel y Accept
...
//Funcion cerrado automatico Edit Menu
document.addEventListener('click', function(event) {
    if (event.target.classList.contains('editProductButton')) {
        event.target.style.display = 'none'; 
        deleteProductButton.style.display = 'none';
    }else if (event.target.classList.contains('cancelEditProductButton')) {
        editMenu.style.display = 'none';
        acceptEditProductButton.style.display = 'none';
        event.target.style.display = 'none';
